...
from db_c import conn_obj
from db_c import cursor_obj
import cloudinary
...
def login_function():
    ...
    with st.form("Login_Form"):
        ...
        if btn :
            query="select * from users4 where email=%s and password = %s"
...
def signup_function():
    ...
    with st.form("SignUp_Form"):
        ...
        if btn:
            query="insert into users4(name,email,password) values(%s,%s,%s)"
